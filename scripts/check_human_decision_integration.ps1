...
$composeFile = Join-Path $projectRoot "compose.human-decision-check.yaml"
...
function Remove-TemporaryEnvironment {
    Remove-Item Env:\AI_AUTOMATION_HUMAN_DB_PASSWORD -ErrorAction SilentlyContinue
    Remove-Item Env:\AI_AUTOMATION_HUMAN_INTAKE_TOKEN -ErrorAction SilentlyContinue
    Remove-Item Env:\AI_AUTOMATION_HUMAN_SESSION_SECRET -ErrorAction SilentlyContinue
    Remove-Item Env:\AI_AUTOMATION_HUMAN_WORKFLOW_TOKEN -ErrorAction SilentlyContinue
}

Write-Host "AI Service Request Automation - human-decision integration check"
Write-Host "Scope: 6 focused groups; fictional data; signed sessions; 0 AI calls."
Write-Host ""
...
$env:AI_AUTOMATION_HUMAN_DB_PASSWORD = (
    [guid]::NewGuid().ToString("N") + [guid]::NewGuid().ToString("N")
)
$env:AI_AUTOMATION_HUMAN_INTAKE_TOKEN = (
    [guid]::NewGuid().ToString("N") + [guid]::NewGuid().ToString("N")
)
$env:AI_AUTOMATION_HUMAN_SESSION_SECRET = (
    [guid]::NewGuid().ToString("N") + [guid]::NewGuid().ToString("N")
)
$env:AI_AUTOMATION_HUMAN_WORKFLOW_TOKEN = (
    [guid]::NewGuid().ToString("N") + [guid]::NewGuid().ToString("N")
)
...
try {
    ...
    if ($build.ExitCode -ne 0) {
        ...
        throw "The human-decision application image could not be prepared."
    }
    ...
    if ($startup.ExitCode -ne 0) {
        ...
        throw "The disposable human-decision services did not become healthy."
    }
    ...
    if ($tests.ExitCode -ne 0) {
        ...
        throw "The human-decision integration groups failed."
    }
    if (-not ($tests.Output -match '^  Human-decision gate: PASS$')) {
        throw "The human-decision completion marker was missing."
    }
    @($tests.Output | Where-Object {
        ([string]$_) -match '^(\[[1-6]/6\]|Human-decision integration summary|  )'
    }) | ForEach-Object { Write-Host ([string]$_) }
    Write-Host ""
    Write-Host "PASS: Human decisions passed their controlled local application check."
}
catch {
    ...
    $logs = Invoke-NativeCapture {
        docker compose --file $composeFile logs --no-color --tail 40 primary-api
    }
